...
exports.afterUploadImage = (req, res) => {
  ...
  const originalUrl = req.file.location; // 원본 주소
  const url = original.replace(/\/original\//, "/thumb/"); // "/original/" 을 "/thumb/"로 변경
  res.json({ url, originalUrl }); // 업로드된 이미지 URL을 프론트로 다시 보내줌.
};
